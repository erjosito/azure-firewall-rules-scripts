...
#         |-> DestinationIpGroups
#         |-> DestinationPorts
# A JSON generated by 'az network firewall policy rule-collection-group list --policy-name $azfw_policy_name -g $rg -o json' has this structure
# root {
# |-> RuleCollections [       **
#     |-> RuleCollectionType   **
#     |-> Rules [
            ...
            if collection['ruleCollectionType'] == 'FirewallPolicyFilterRuleCollection':
                ...
                for rule in collection['rules']:
                    if rule['ruleType'] == 'NetworkRule':
                        if verbose:
                            print('DEBUG: generating network rule', rule['name'])
                        new_rule = {}
                        ...
                        if verbose:
                            print('DEBUG: ignoring rule', rule['name'], 'of type', rule['ruleType'])
...
# First we verify the JSON has the right syntax
# 1. If the policy object is an array and the 1st element contains the key 'ruleCollections',
#    it was probably generated with the CLI (az network firewall policy rule-collection-group list)
if isinstance(policy, list) and 'ruleCollections' in policy[0]:
    if verbose:
...
# 2. Otherwise, we expect the JSON object to be a dictionary and to contain the key 'NetworkRuleCollections'
elif isinstance(policy, dict) and 'NetworkRuleCollections' in policy:
    if verbose:
        print("DEBUG: It looks like the policy is correct and contains the key 'NetworkRuleCollections'")
# 3. Else, the policy is not correct and we stop
else:
    print("ERROR: the JSON file has an unexpected format.")
    print("       Please generate the JSON file with the commands 'az network firewall policy rule-collection-group list' or")
    print("       with 'az group export' to export a classic firewall configuration.")
    sys.exit(1)
...
# Print some info
if 'NetworkRuleCollections' in policy:
    ...
    if summary_output: print("No network rule collections to analyze")
